import 'package:bookly_app/conatants.dart';
import 'package:flutter/material.dart';

abstract class Styles {
  static const textStyle21 =
      TextStyle(fontWeight: FontWeight.w600, fontSize: 21);

  static const textStyle22 = TextStyle(
      fontWeight: FontWeight.w500, fontSize: 22, fontFamily: kGtSectraFine);

  static const textStyle18 =
      TextStyle(fontWeight: FontWeight.normal, fontSize: 18);

  static const textStyle14 =
      TextStyle(fontWeight: FontWeight.normal, fontSize: 14);

  static const textStyle16 =
      TextStyle(fontWeight: FontWeight.w500, fontSize: 16);

  static const textStyle30 = TextStyle(
      fontWeight: FontWeight.bold, fontSize: 30, fontFamily: kGtSectraFine);
}
